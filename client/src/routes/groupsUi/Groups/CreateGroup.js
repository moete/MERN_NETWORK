...
export  class  CreateGroup extends Component {
    ...
    render() {
      ...
      <Form>
        ...
        >
          <Input  placeholder="Web development,Mobile development,Informations security ..." id="success"/>
        </FormItem>

        <div > 
                <label>Admins                      </label><br/>
                <select ref="adminsInput"
                    required
                    className="form-control"
                    value={this.state.admins[0]}
                    onChange={this.onChangeAdmins}>
                    {
                      this.state.admins.map(function(membersn) {
                        return <option 
                          key={membersn}
                          value={membersn}>{membersn}
                          </option>;
                      })
                    }
                </select>
        ...
        <div > 
                ...
                <select ref="membersInput"
                    required
                    className="form-control"
                    value={this.state.members[0]}
                    onChange={this.onChangemembers}>
                    {
                      this.state.members.map(function(membersn) {
                        return <option 
                          key={membersn}
                          value={membersn}>{membersn}
                          </option>;
        ...
        >
          <Input  placeholder="type private to create a private group" id="success"/>
          <h4>the default value of privacy is public .</h4>
        </FormItem>
      </Form>
      <Button type="primary" htmlType="submit" onClick={this.onSubmit}>Create</Button>
      <br/><br/>
