...
export default function Saved({ job, count }) {
    ...
    const openNotification = () => {
        notification.open({
            ...
            description:
                `Maybe your next job will be located in ${job.job_id.location}`,
            onClick: () => {
        ...
        <Card className="mb-3">
            <Card.Body  >
                <div className="d-flex justify-content-between">
                    <div>
                        <Card.Title   >
                            {job.job_id.title} - <span className="text-muted font-weight-light"></span>
                        </Card.Title>
                        <Card.Subtitle className="text-muted mb-2">
                            {new Date(job.created_at).toLocaleDateString()}
                        </Card.Subtitle>
                        <span className="gx-mr-3">
                            {job.job_id.employees_needed}  <span className="gx-text-grey">Employess needed</span>
                        </span>
